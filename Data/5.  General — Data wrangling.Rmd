...
```{r}
arg_raw <- read_rds(here("Data", "Exports raw", "arg.rds"))
chl_raw <- read_rds(here("Data", "Exports raw", "chl.rds"))
```
...
exports_raw %<>% 
  filter(partner_iso %notin% c("ARG", "CHL")) %>% 
  bind_rows(arg_raw, chl_raw)
```
...
```{r}
partners <- 
  read_csv("https://raw.githubusercontent.com/gafnts/gravity-model/main/Data/World%20Bank%20raw/partners_wdi.csv") %>% 
...
```{r}
guatemala <- 
  read_csv("https://raw.githubusercontent.com/gafnts/gravity-model/main/Data/World%20Bank%20raw/guatemala_wdi.csv") %>% 
  select(year:iso3c, capital:latitude) %>% 
