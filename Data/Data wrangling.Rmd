...
```{r}
write_rds(countries, file = paste0(here("Data", "Main data frames"), "/countries.rds"))
```

```{r}
# Test
trade %>% 
  left_join(countries, by = c("partner_iso", "partner")) %>% 
  View()
```

## (c) Structural gravity variables

### Import World Bank dataframe

```{r}
worldbank <- 
  read_rds("https://github.com/gafnts/gravity-model/blob/main/Data/World%20Bank%20raw/worldbank.rds?raw=true") %>% 
...
First of all, GUF (French Guyana) is not a country recognized by the WDI database, so it has to go from the trade.rds and countries.rds data frames.

As of this moment, Guatemala isn't contained in the worldbank.rds tibble, so I think that the country needs to have its own data frame. But take a moment to think about the best way to do it knowing that everything has to work in the econometric framework of the gravity model.

Finally, an EDA needs to be done for the worldbank.rds in order to get sure all the information is available for every country.
