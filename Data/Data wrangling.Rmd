```{r}
pacman::p_load(tidyverse, here)
```

# General data wrangling

## (a) Trade data frame

### Loading exports and imports

```{r}
# Exports
setwd(here("Data", "Exports raw"))

exports_raw <- 
  dir(pattern = "*.rds") %>% 
  map(read_rds) %>% 
  reduce(bind_rows)

# Imports
setwd(here("Data", "Imports raw"))

imports_raw <-
  dir(pattern = "*.rds") %>%
  map(read_rds) %>%
  reduce(bind_rows)
```

### Binding and cleaning

```{r}
trade <- 
  bind_rows(exports_raw, imports_raw) %>% 
  select_if(~ !any(is.na(.))) %>% 
  select(-c(classification, period, period_desc, 
            is_leaf_code, trade_flow_code, flag, 
            aggregate_level, reporter_code, partner_code)) %>% 
  group_by(trade_flow) %>% 
  arrange(year, trade_flow, partner, commodity) %>% 
  ungroup()
```

### Writing tibble

```{r}
setwd(here())
write_rds(trade, file = paste0(here("Data", "Main data frames"), "/trade.rds"))
```

## (b) Country names data frame

```{r}
codes <-
  read_csv("https://git.io/v564u") %>%
  select(name,
         partner_iso = `alpha-3`,
         subregions = `sub-region`,
         int_region = `intermediate-region`)
```

Guyana is repeated twice, which means there are problems in the trade.rds tibble that need to be corrected. Consider extract the Comtrade data again, but this time paying close attention to UN's geoscheme. UN geoscheme: [<https://en.wikipedia.org/wiki/United_Nations_geoscheme#Americas>](https://en.wikipedia.org/wiki/United_Nations_geoscheme#Americas){.uri}

```{r}
names <- 
  tribble(~name,
        # Central America and Mexico
        "Belize", "Costa Rica", "Panama", "Honduras",
        "Nicaragua", "El Salvador", "Mexico",
        # South America
        "Argentina", "Bolivia (Plurinational State of)",  "Brazil", "Chile",
        "Colombia", "Ecuador", "French Guiana", "Venezuela", "Guyana",
        "Paraguay", "Peru", "Uruguay", "Suriname",
        # Caribbean
        "Dominican Rep.", "Cuba", "Haiti", "Trinidad and Tobago",
        "Antigua and Barbuda", "Dominica", "Grenada", "Jamaica", "Barbados",
        "Bahamas", "Saint Kitts and Nevis", "Saint Vincent and the Grenadines", 
        "Saint Lucia")
```

```{r}
countries <- codes %>% 
  inner_join(names, by = "name")
```

Problems with names:

```{r}
setdiff(names, countries %>% select(name))
```

```{r}
codes %>% 
  filter(subregions == "Latin America and the Caribbean") %>% 
  arrange(int_region) %>% 
  group_by(int_region) %>% 
  summarise(n = length(int_region))
```
