...
Notes: Count data instead of sum of value
...
exports <- 
  ...
  select(!c(trade_flow, commodity_code, commodity)) |> 
  relocate(hs, partner_iso, trade_value_usd) |> 
  group_by(hs, partner_iso) |> 
  summarise(value = sum(trade_value_usd)) |> 
  pivot_wider(names_from = hs, values_from = value) |> 
  # janitor::clean_names() |> 
...
```{r}
pca <- prcomp(exports[, -1], center = TRUE, scale. = TRUE)
summary(pca)
...
pca_exports <- 
  recipe(~., data = exports) |> 
  update_role(hs, partner_iso, new_role = 'id') |> 
...
juice(prep_exports) %>%
  ...
  geom_point(aes(color = hs), alpha = 0.7, size = 2) +
