...
Wouldn't be easier to just extract trade data country by country?\
If that doesn't wotks, well... AG2.\
There is no data available in the Comtrade database for KNA (1995-2000), ATG (1995-2000), PRY (1995), GRD (1996, 1997).\
Incomplete series in imports: DMA, GRD, LCA, VCT\
\
**Remember to:**\
...
```{r}
# These are the countries and years for which something's wrong in total_exports
# The problem is there is TMI in those years, so comtrade defaults to just not extracting 'em
transaction_costs %>% anti_join(total_exports) %>% View

# Remove VEN from trade flows data frame
total_exports %>% anti_join(transaction_costs) %>% View
```
